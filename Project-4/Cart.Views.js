...
{
	var purchase_quantity_flag=false;
	var available_quantity_flag=false;
	'use strict';
	var Views = {}
	,	colapsibles_states = {};
	...
	,	events: {
		...
		,   'click [data-action="custom-recent"]': 'addToCart'
		,   'click [data-action="custom-notify"]':'callNotifyMe' 
	...
	,	showContent: function ()
		{
			return this.options.application.getLayout().showContent(this, true).done(function (view)
			{
				...
				{
					this.application.showRelatedItems && this.application.showRelatedItems(view.model.getItemsIds(), related_items_placeholder);	
				}
				...
				{
					this.application.showRelatedItems && this.application.showCorrelatedItems(view.model.getItemsIds(), correlated_items_placeholder);	
				}
	...
	,	applyPromocode: function (e)
		{
			...
			this.model.save({
				...
				{
					self.showContent();
					var promocodeValue=options.promocode; 
				    alert('promo code value is'+promocodeValue);
					jQuery.get("/app/site/hosting/scriptlet.nl?script=571&deploy=2&promocode="+promocodeValue, function(data) {
						var bogoData=JSON.stringify(data).replace(/\"/g, "").split("#");
     					var Xitem=bogoData[0];
     					var Yitem=bogoData[1];
     					var YitemQuant=jQuery("#limitedinfo-"+Xitem).attr("item-quant");
     					//limitedinfo-
     					alert(Xitem+"===>>>"+Yitem);
     					if(YitemQuant>2)
     						YitemQuant=2;
     					//alert(jQuery("#limitedinfo-"+Xitem).attr("item-quant"));

					    var url="http://lionelsandbox.com/app/site/backend/intl/additemtocart.nl?c=445393&buyid="
					    				+Yitem+"&qty="+YitemQuant+"&&showcart=T";
					    alert(url);
	...
	,	addToCart: function (e)
		{
			console.log(' In add to cart Logic');
			var flag=jQuery(e.target).attr('custom-flag');
			var customData=jQuery(e.target).attr('custom-data');
			console.log(flag+"==="+customData);
			var url="";
			if(flag=="addToCart")
			{
			  url=SC.SESSION.touchpoints.home +"/app/site/backend/intl/additemtocart.nl?c=445393&buyid="+customData+"&qty=1&&showcart=T";
			}
			else if(flag=="Matrix"){
		      url=SC.SESSION.touchpoints.home +customData;
			}
			else if(flag=="Gift")
			{
			  url=SC.SESSION.touchpoints.home +customData;	
			}
			//console.log('in custom add to cart page'+url);
    		window.location.href = url;
		}
		
		// view.callNotifyMe: 
		// Takes care of Notifying the current model item and add to custom Notification List	
   ,	callNotifyMe: function (e)
		{
			 console.log(' In Notify me Block');
			 var login=SC.SESSION.touchpoints.login;
			 var itemId=jQuery(e.target).attr("custom-data");
			 login+='&origin=home&pdpvalue=notifyme-'+itemId+'&origin_hash='+ Backbone.history.fragment;
			 console.log("login url==>>"+login);
    
	         if(jQuery(".my-account-title").html()=="Your Account")
			  {
			  	var itemId=jQuery(e.target).attr("custom-data");
			  	console.log(itemId);
				 var notifyListId=jQuery('#notifyListId').val();
                                 var ssurl=SC.Utils.getAbsoluteUrl("services/NotifyCall.ss")+"?c=445393&n=7&itemId="+itemId+"&notifyListId="+notifyListId+"&is=newNotifyListItem"; 
				 //var url="http://lionelsandbox.com/c.445393/shopflow-1-05-0/services/NotifyCall.ss?c=445393&n=7&itemId="+itemId+"&notifyListId="+notifyListId+"&is=newNotifyListItem"; 
				 console.log(ssurl);
				  jQuery.get(ssurl
					  , function(data, status) {
					    ...
					    setTimeout(function(){ jQuery('#message').fadeOut("slow") }, 3000);
				 });
			  }
			  else
   ...
   ,	proceedToCheckOut: function (e)
	{
		var msgvalue=jQuery('.msgtxt1').html();
		console.log('msgvalue'+msgvalue);
	   var sessioncheckoutURL=SC.SESSION.touchpoints.checkout;
	    if(msgvalue===undefined || msgvalue===null){	
	    	console.log('session_proceed_flag');
	    	window.location.href=sessioncheckoutURL;
	     		}
}
